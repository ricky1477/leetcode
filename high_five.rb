...
# def high_five(items)
#   # Hash with default of empty array
#   scores = Hash.new []
#   items.each do |student_id, score|
#     #p [student_id,score]
#     # byebug
#     if scores.key?(student_id)
#       # scores[student_id] << score
#       # scores[student_id] = {scores: [score], average: 0}
#       if scores[student_id][:scores].length > 4
#         #p 'greater than 4 scores'
#         #p scores[student_id][:scores].min
#         if scores[student_id][:scores].min < score
#           scores[student_id][:scores].delete_at(scores[student_id][:scores].find_index(scores[student_id][:scores].min))
#           # add score
#           scores[student_id][:scores] << score
#         end
#       else 
#         # add score
#         scores[student_id][:scores] << score
#       end
#       # calculate average
#       scores[student_id][:average] = scores[student_id][:scores].sum / scores[student_id][:scores].length
#     else
#       scores[student_id] = {scores: [score], average: score}
#     end
#     # p '----'
#     # p scores
#   end
#   scores.map{|k, v| [k, v[:average]]}
# end
